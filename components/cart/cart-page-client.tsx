        ...
        <div className="card-surface p-6">
          {items.length ? (
            <div className="space-y-4">
              {items.map((item) => (
                <div key={item.key} className="grid gap-4 rounded-[1.5rem] border border-border p-4 md:grid-cols-[auto_1fr_auto]">
                  <div className="relative h-28 w-28 overflow-hidden rounded-[1.2rem] bg-sand-strong">
                    <Image src={item.image} alt={item.name} fill className="object-cover" />
                  </div>
                  <div className="space-y-3">
                    <Link href={`/shop/${item.slug}`} className="display-font text-2xl font-semibold text-ink">
                      {item.name}
                    </Link>
                    ...
                    <p className="font-semibold text-forest">{formatPriceFromCents(item.priceCents)}</p>
